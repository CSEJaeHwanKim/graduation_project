...
#define ZERO_POINT_VOLTAGE (0.125) // sensor each different 6
...
unsigned char waterSensor;          // 전류변화값 변수선언
...
void loop()
{
    ...
    if(millis() - vibration_timer > 20000)
    {
      vibration_timer = millis();
      vibration = 0;
    }
    if( temp_1 < 30 || vibration < 5 || waterSensor < 50 )
    {
      digitalWrite(redLedPin, LOW);
      digitalWrite(yellowLedPin, LOW);
      digitalWrite(greenLedPin, HIGH);
    }
    if( (temp_1 >= 30 && temp_1 < 40) || (vibration >= 5 && vibration < 200) || (waterSensor >= 50 && waterSensor < 150))
    {
      digitalWrite(greenLedPin, LOW);
      digitalWrite(redLedPin, LOW);
      digitalWrite(yellowLedPin, HIGH);
    }
    if(temp_1 >= 40 || vibration >=  200 || waterSensor >= 150 )
    {
      digitalWrite(greenLedPin, LOW);
      digitalWrite(yellowLedPin, LOW);
